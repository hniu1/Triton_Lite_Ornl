...
#SBATCH --mem=64G
#SBATCH -t 01:00:00
#SBATCH -o ./slurm_output/stage1-v4-smoke-%j.out
#SBATCH -e ./slurm_output/stage1-v4-smoke-%j.err
...
python -u stage1_train.py \
  ...
  --output-dir results/stage1_timestamp_v4_smoke --test-events D030 \
  --batch-size 16 --train-batches-per-epoch 10 --eval-batches 5 \
  --eval-time-stride 24 --epochs 1 --early-stop-patience 1 \
  --learning-rate 3e-5 --num-workers 0 \
