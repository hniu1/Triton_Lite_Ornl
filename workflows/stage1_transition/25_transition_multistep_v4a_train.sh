...
#SBATCH --mem=96G
#SBATCH -t 04:00:00
#SBATCH -o ./slurm_output/stage1-transition-multistep-v4a-%j.out
#SBATCH -e ./slurm_output/stage1-transition-multistep-v4a-%j.err
...
python -u stage1_transition_multistep_v2_train.py \
  ...
  --output-dir results/stage1_transition_multistep_v4a \
  --history-states 2 --use-activity-gate --activity-gate-initial-bias 2.0 \
  --rollout-steps 12 --epochs 3 --batch-size 8 \
  --train-batches-per-epoch 250 --eval-batches 100 --eval-time-stride 6 \
  --learning-rate 2e-6 --predicted-state-probability-start 0.15 \
  --predicted-state-probability-end 0.40 \
  ...
  --num-workers 2 --device cuda --disable-cudnn
