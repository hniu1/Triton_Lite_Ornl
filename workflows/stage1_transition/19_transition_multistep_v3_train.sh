...
#SBATCH --mem=96G
#SBATCH -t 24:00:00
#SBATCH -o ./slurm_output/stage1-transition-multistep-v3-%j.out
#SBATCH -e ./slurm_output/stage1-transition-multistep-v3-%j.err
...
python -u stage1_transition_multistep_v2_train.py \
  ...
  --output-dir results/stage1_transition_multistep_v3 \
  --rollout-steps 12 --epochs 3 --batch-size 8 \
  --train-batches-per-epoch 250 --eval-batches 100 --eval-time-stride 6 \
  --learning-rate 2e-6 --predicted-state-probability-start 0.35 \
  --predicted-state-probability-end 0.75 \
